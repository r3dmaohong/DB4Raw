...
North <- c("台北市","新北市","基隆市","桃園市","桃園縣","新竹市","新竹縣")
Mid   <- c("苗栗縣","台中市","彰化縣","南投縣","雲林縣")
South <- c("嘉義縣","嘉義市","台南市","高雄市","屏東縣")
East  <- c("宜蘭縣","花蓮縣","台東縣","澎湖縣","金門縣","連江縣")
Out   <- c(North, Mid, South, East)
total.data[ substr(total.data[,工作地點],1,3) %in% North, area.work:="北部地區"]
total.data[ substr(total.data[,工作地點],1,3) %in% Mid, area.work:="中部地區"]
total.data[ substr(total.data[,工作地點],1,3) %in% South, area.work:="南部地區"]
total.data[ substr(total.data[,工作地點],1,3) %in% East, area.work:="東部與離島地區"]
total.data[ !(substr(total.data[,工作地點],1,3) %in% Out) , area.work:="非台灣地區"]

unique(total.data$area.work)
## Check areas which are outside Taiwan
total.data[ !(substr(total.data[,工作地點],1,3) %in% Out) , 工作地點] %>% substr(., 1, 3) %>% table
...
total.data$希望工作性質 %>% table
total.data <- total.data %>% filter(., 希望工作性質=="全職" | 希望工作性質=="中高階")

## Working area transfer
total.data$area.work <- NA
"北部地區"       -> total.data[which(substr(total.data[,"希望上班地區名稱"],1,3) %in% c("台北市","新北市","基隆市","桃園市","桃園縣","新竹市","新竹縣")),"area.work"]
"中部地區"       -> total.data[which(substr(total.data[,"希望上班地區名稱"],1,3) %in% c("苗栗縣","台中市","彰化縣","南投縣","雲林縣")),"area.work"]
"南部地區"       -> total.data[which(substr(total.data[,"希望上班地區名稱"],1,3) %in% c("嘉義縣","嘉義市","台南市","高雄市","屏東縣")),"area.work"]
"東部與離島地區" -> total.data[which(substr(total.data[,"希望上班地區名稱"],1,3) %in% c("宜蘭縣","花蓮縣","台東縣","澎湖縣","金門縣","連江縣")),"area.work"]
"非台灣地區"     -> total.data[which(substr(total.data[,"希望上班地區名稱"],1,3) %in% c("台北市","新北市","基隆市","桃園市","桃園縣","新竹市","新竹縣",
                                                                       "苗栗縣","台中市","彰化縣","南投縣","雲林縣", 
                                                                       "嘉義縣","嘉義市","台南市","高雄市","屏東縣", 
                                                                       "宜蘭縣","花蓮縣","台東縣","澎湖縣","金門縣","連江縣")==FALSE),"area.work"]
## Check areas which are outside Taiwan
total.data[which(substr(total.data[,"希望上班地區名稱"],1,3) %in% c("台北市","新北市","基隆市","桃園市","桃園縣","新竹市","新竹縣",
                                                        "苗栗縣","台中市","彰化縣","南投縣","雲林縣", 
                                                        "嘉義縣","嘉義市","台南市","高雄市","屏東縣", 
                                                        "宜蘭縣","花蓮縣","台東縣","澎湖縣","金門縣","連江縣")==FALSE),"希望上班地區名稱"] %>% table
...
top10beloved.job <- total.data %>% filter(., 希望職務小類名稱!="工讀生") %>% group_by(date, area.work, 希望職務小類名稱) %>% 
  summarize(.,Freq=n()) %>% arrange(.,date, area.work, -Freq) 
...
top10beloved.job$Freq <- sapply(1:nrow(top10beloved.job), function(x){
  top10beloved.job$Freq[x] - standard.top10beloved.job$Freq[which(standard.top10beloved.job$希望職務小類名稱==top10beloved.job$希望職務小類名稱[x] & standard.top10beloved.job$area.work==top10beloved.job$area.work[x])]
})
...
for(i in 1:nrow(dpt.match)){
  total.data$dpt[which(total.data$最高學歷_科系小類名稱==dpt.match[i,3])] <- dpt.match[i,2]
}

dpt.top10beloved.job <- total.data %>% filter(., 希望職務小類名稱!="工讀生") %>% group_by(date, dpt, 希望職務小類名稱) %>% 
...
##
##地區是否要再做處理
top10beloved.area <- total.data %>% filter(., 希望職務小類名稱!="工讀生") %>% mutate(., 希望上班地區名稱=substr(希望上班地區名稱, 1, 3)) %>% group_by(date, 希望上班地區名稱) %>% 
  summarize(.,Freq=n()) %>% arrange(.,date, -Freq) 
...
##地區是否要再做處理
dpt.top10beloved.area <- total.data %>% filter(., 希望職務小類名稱!="工讀生") %>% mutate(., 希望上班地區名稱=substr(希望上班地區名稱, 1, 3)) %>% group_by(date, dpt, 希望上班地區名稱) %>% 
  summarize(.,Freq=n()) %>% arrange(.,date, -Freq) 
...
## Top 10 most beloved jobs
demand_job_data <- total.data %>% select(資料時間, ID1111職務小類名稱, 地理區域, 新_1111行業中類名稱)
